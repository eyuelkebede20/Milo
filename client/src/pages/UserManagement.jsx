  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        setEvents(data);
        if (data.length > 0) {
          setForm((prev) => ({ ...prev, assignedEventId: data[0]._id }));
        }
      } catch (err) {
      ...
      <div className="card bg-base-100 border-4 border-base-content shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.2)] rounded-none">
        <div className="card-body">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              disabled={loading}
              className={`btn btn-primary w-full mt-4 rounded-none border-2 border-base-content font-black text-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] ${loading ? "loading" : ""}`}
            >
              CREATE ACCOUNT
            </button>
          </form>
